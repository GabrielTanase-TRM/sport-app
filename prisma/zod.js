...
async function transform(datamodel) {
	...
	function toZodField(field, enums) {
		...
		const isOptional = !field.isRequired || field.hasDefaultValue || field.isUpdatedAt;
	...
	for (const model of datamodel.models) {
		...
		const content = `
			...
			export type Include = Prisma.${model.name}Include;
			...
			export type QueryOptions = {
				orderBy?: Prisma.${model.name}OrderByInput;
				limit?: number;
				offset?: number;
				include?: Include;
				fields?: Fields;
